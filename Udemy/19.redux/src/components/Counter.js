...
import { counterActions } from '../store/redux_slice';
import classes from './Counter.module.css';

const Counter = () => {
  const dispatch = useDispatch();
  const counter = useSelector((state) => state.counterSlice.counter);
  const show = useSelector((state) => state.counterSlice.view);
  ...
  return (
    <main className={classes.counter}>
      ...
      <div>
        ...
        <button style={{margin: '1rem'}} onClick={increaseHandler}>Increase by 10</button>
